...
for data_name in data_list:
   ...
   result_vec = np.zeros(len(sn_opt))  

   feature_subsets, group_sizes = split_features(data, T=60)

   # iterate snn and sa threshold
   for i, a1 in enumerate(sn_opt):
      model = CSSGT(in_channels=group_sizes, threshold=0.7, snn_type=a1, if_multi_output= False).to(device)
      ...
      df_loss.to_csv(f'results/ablation_sn_{data_name}/loss/sn_loss_{data_name}_{a1}.csv', index=False, header=False)

      # save the model_acc_curve
      df_acc = pd.DataFrame(model_acc_curve)
      df_acc.to_csv(f'results/ablation_sn_{data_name}/accuracy/sn_acc_{data_name}_{a1}.csv', index=False, header=False)

   # save the result matrix
   df_matrix = pd.DataFrame(result_vec)
   df_matrix.to_csv(f'results/ablation_sn_{data_name}/sdga_all_models_matrix_{data_name}.csv', index=False, header=False)
