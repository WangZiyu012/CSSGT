...
for data_name in data_list:
   ...
   print(f"Training model on {data_name}")
   model_acc_curve = []
   model_loss_curve = []
   ...
   df_loss.to_csv(f'results/main_{data_name}/loss/main_loss_{data_name}.csv', index=False, header=False)

   # save the model_acc_curve
   df_acc = pd.DataFrame(model_acc_curve)
   df_acc.to_csv(f'results/main_{data_name}/accuracy/main_acc_{data_name}.csv', index=False, header=False)
