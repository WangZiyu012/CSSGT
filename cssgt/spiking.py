...
def creat_snn_layer(v_threshold=0.7, snn="PLIF"):
    ...
    if snn == "PLIF":
        return PLIF(v_threshold=v_threshold, detach=True)
    elif snn == "IF":
...
class QIF(nn.Module):
    ...
    def forward(self, dv):
        # 1. charge
        self.v += dv + self.alpha * (self.v - 1) * self.v
        # 2. fire
        spike = self.surrogate(self.v, self.v_threshold, self.alpha)
        if self.detach:
            detached_spike = spike.detach()
...
class RF(nn.Module):
    def __init__(
        self, v_threshold=1.0,omega = 10, alpha=-1, v_reset=0.0, surrogate="triangle", detach=True,
    ):
        super().__init__()
        self.v_threshold = v_threshold
        self.v_reset = v_reset
    ...
    def forward(self, dv):
        # 1. charge
        self.v += dv + self.alpha * self.v - self.omega * self.u
        self.u +=  self.omega * self.v + self.alpha * self.u
        ...
        # 3. reset
        self.v = (1 - detached_spike) * v + detached_spike * self.v_reset
        self.u = (1 - detached_spike) * u + detached_spike * self.u_reset
        return spike
...
class Izhikevich(nn.Module):
    ...
    def forward(self, dv):
        ...
        # 1. charge
        self.v +=  0.01*self.tau *self.v * self.v + 5*self.v + 140 - self.u + dv
        self.u +=  self.alpha * (0.2*self.v - self.u)
        ...
        # 3. reset
        self.v = (1 - detached_spike) * v + detached_spike * self.v_reset
        self.u = (1 - detached_spike) * u + detached_spike * (self.u_reset-1)
        return spike
